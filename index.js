/**
 * 👑 AlokOTP Pro — FULL-STACK WEB APP EDITION (Grizzly Clone)
        ...
        async function checkOTP() {
            ...
            try {
                ...
                if(data.status === 'OK') {
                    ...
                    tg.showAlert("⏳ Still waiting for SMS. Please wait...");
                } else {
        ...
        async function cancelOrder() {
            ...
            tg.showConfirm("Are you sure you want to cancel? Funds will be refunded.", async (ok) => {
                ...
                try {
                    ...
                    if(data.success) {
                        tg.showAlert("✅ Order Cancelled! Refund added to your wallet.");
                        initUser();
                        showMain();
                    } else {
                        tg.showAlert("⚠️ Could not cancel: " + data.message);
                    }
                } catch(e) {}
            });
        }
  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === "POST") {
      try {
        ...
        // 🟢 INTERNAL WEB APP API (Frontend to Backend)
        if (body.type === 'api') {
        ...
        if (body.message) {
            ...
            if (userId === ADMIN_ID.toString()) {
                if (text.startsWith("/add")) {
                    ...
                    await sendTgMessage(chatId, `✅ Deposit UPI updated to: \`${upi}\``);
                }
            }
        }

        // 🔵 TELEGRAM CALLBACK QUERIES (Inline Buttons)
        if (body.callback_query) {
            ...
            if (cb.data === "admin_panel" && userId === ADMIN_ID.toString()) {
                ...
                const txt = `🛡️ *Admin Control Center*\n\n🐻 *Grizzly API Balance:* \`${gBal}\`\n🏦 *Current UPI:* \`${upi}\`\n📈 *Profit Margin:* \`${PROFIT_PERCENTAGE * 100}%\`\n\n*Quick Commands:*\n➕ Add Funds: \`/add <UserID> <$Amount>\`\n🏦 Change UPI: \`/setupi <upi@bank>\``;
